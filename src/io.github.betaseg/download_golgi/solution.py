...
def run():
    ...
    dl_path = download_if_not_exists(
        "https://syncandshare.desy.de/index.php/s/FikPy4k2FHS5L4F/download/data_golgi.zip", "data_golgi.zip")

    # extract data_granules.zip
    if not Path(dl_path.parent).joinpath("data_golgi").exists():
        with zipfile.ZipFile(dl_path, 'r') as zip_ref:
            zip_ref.extractall(dl_path.parent)

    # copy to target path
    shutil.copytree(dl_path.parent.joinpath("data_golgi"), get_args().target_path)
